...
[AddComponentMenu("Playground/Movement/Move Camera With Arrows")]
...
public class MoveCameraWithArrows : MonoBehaviour
{
	...
	{	
        ...
        if (Input.GetKeyDown("i")) y = -1f;
        if (Input.GetKeyDown("k")) y = 1f;
        if (Input.GetKeyDown("j")) x = -1f;
        if (Input.GetKeyDown("l")) x = 1f;
        cameraMovement = new Vector3(x, y, z);
    }
	...
	void FixedUpdate ()
	{
        ...
        Debug.Log("x-liike " + movement.x);
        Debug.Log("y-liike " + movement.y);
    }
    //
